        ...
        {filteredSchools.length === 0 ? (
          ...
          filteredSchools.map((school) => (
            <motion.div 
              ...
              className="bg-white border border-slate-200 rounded-2xl flex items-center gap-4 p-4 sm:p-5 hover:shadow-lg transition-all duration-300 cursor-pointer w-full group"
            >
              <div className="w-16 h-16 md:w-20 md:h-20 shrink-0 bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm flex items-center justify-center">
                <img src={school.logoUrl} className="max-w-full max-h-full object-contain p-1.5 group-hover:scale-110 transition-transform duration-500" alt={school.name} />
              </div>
              
              <div className="flex-1 flex flex-col justify-center min-w-0">
                <h3 className="text-base md:text-lg font-serif italic font-bold text-slate-900 truncate group-hover:text-moroccan-green transition-colors">{school.name}</h3>
                
                <div className="flex items-center gap-2 mt-1 mb-1 text-[9px] sm:text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-500 flex-wrap">
                  <span className="text-moroccan-green">{school.type}</span>
                  <span className="hidden sm:block w-1 h-1 bg-slate-300 rounded-full"></span>
                  <div className="flex items-center gap-1">
                    <MapPin size={12} className="text-moroccan-red" />
                    <span className="truncate">{school.city}</span>
                  </div>
                </div>
              </div>

              <button className="shrink-0 px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 font-bold text-[10px] sm:text-xs uppercase tracking-widest group-hover:bg-moroccan-green group-hover:text-white group-hover:border-moroccan-green transition-all duration-300 flex items-center gap-2 active:scale-95 shadow-sm">
